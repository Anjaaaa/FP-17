In diesem Kapitel werden als ersten die physikalischen Grundlagen des Versuchs erläutert, dann auf den eigentlichen Versuchsaufbau eingegangen und zuletzt noch die konkrete Funktionsweise des HeNe-Lasers erklärt.
...
\begin{itemize}
	...
	\item Induzierte Emission: Das Elektron ist auf dem oberen Niveau. Ein einfallendes Photon, das die Energie $E$ hat, verursacht die Emission eines zweiten Photons mit Energie $E$ beim Übergang des Elektrons zum unteren Niveau.
...
\end{align}
Die Energiedichte des einfallenden Strahlungsfelds ist dabei $\rho(x)$ und $A_{21}, B_{12}, B_{21}$ die Einsteinkoeffizienten und dienen als Maß für die Wahrscheinlichkeit eines Übergangs. \\
...
\end{align}
wenn $\Delta N(0) = -N$, d.h. alle Atome angeregt sind, und die Niveaus nicht entartet, d.h. $B_{12}=B_{21}=B$, sind \cite{Einsteinkoeff}. Für lange Zeiten konvergiert $\Delta N$ gegen $N\slash(2\frac{B\rho}{A_{21}}+1) > 0$, d.h. eine dauerhafte Besetzungsinversion kann mit einem Zweiniveausystem nicht erreicht werden.
...
\begin{align}
	...
	&g_\text{flat}(L) = 1-\frac{L}{r_1} &&\text{\small wenn der zweite planar ist.}\label{eq:StabFlat}
\end{align}
...
\end{align}
modelliert werden, wobei $\theta = \omega_0\lambda\slash\pi$ die Divergenz des Strahls ist. \\
\ \\
